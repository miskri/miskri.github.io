...
cardInfoRenderer = (card) => {
    ...
    if (card.classList.contains("tv")) renderMoreInfo(dbServiceUnit.getTvCard(card.id));
    else if (card.classList.contains("movie")) renderMoreInfo(dbServiceUnit.getMovieCard(card.id)); // movie card rendering
};
...
renderMoreInfo = (promise) => {
    promise.then(response => {
        ...
        releaseYear.textContent = response.release_date ? prettyDateView(response.release_date) : prettyDateView(response.first_air_date);
...
renderListItems = (source, destination) => {
